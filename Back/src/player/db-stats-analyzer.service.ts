...
export class DBStatsAnalyzerService {
  ...
  // 클립 데이터에서 DB 스탯 추출
  async analyzeDBStats(clips: ClipData[], playerId: string): Promise<DBStats> {
    ...
    for (const clip of clips) {
      ...
      // 액션별 스탯 집계
      this.analyzeDefensiveAction(clip, carrier, dbStats);

      // SignificantPlays 확인
      this.analyzeSignificantPlays(clip, dbStats);

      // 색 확인 (PlayType이 'Sack'이고 해당 DB가 Carrier에 있는 경우)
      if (clip.PlayType === 'Sack') {
        dbStats.sacks++;
      }

      // 수비 터치다운 확인 (DB가 Carrier이고 터치다운인 경우)
  ...
  // 수비 액션 분석
  private analyzeDefensiveAction(clip: ClipData, carrier: any, stats: DBStats): void {
  ...
  // SignificantPlays 분석
  private analyzeSignificantPlays(clip: ClipData, stats: DBStats): void {
