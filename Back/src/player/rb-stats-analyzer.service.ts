  ...
  async analyzeRbStats(clips: ClipData[], playerId: string): Promise<RbStats> {
    ...
    for (const clip of clips) {
      ...
      switch (carrier.action.toLowerCase()) {
        case 'rush':
          this.analyzeRushingPlay(clip, rbStats, yards, hasTouchdown);
          break;
        case 'fumble_lost':
          rbStats.fumblesLost++;
          break;
      ...
      switch (clip.PlayType) {
        case 'Pass':
          this.analyzeReceivingPlay(clip, rbStats, yards, hasTouchdown);
          break;
        case 'NoPass':
          // 타겟되었지만 캐치 실패
          rbStats.target++;
          break;
        case 'Kickoff':
          this.analyzeKickReturnPlay(clip, rbStats, yards, hasTouchdown);
          break;
        case 'Punt':
          this.analyzePuntReturnPlay(clip, rbStats, yards, hasTouchdown);
          break;
      }
