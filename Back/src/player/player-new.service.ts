...
export class PlayerNewService {
  constructor(
    @InjectModel(PlayerNew.name) private playerNewModel: Model<PlayerNewDocument>,
  ) {}

  // 선수 프로필 조회
  async getPlayerProfile(playerKey: string): Promise<PlayerNewDocument | null> {
    return this.playerNewModel.findOne({ playerKey }).exec();
  }

  // 새 선수 생성
  async createPlayer(createPlayerDto: CreatePlayerNewDto): Promise<PlayerNewDocument> {
  ...
  ): Promise<PlayerNewDocument | null> {
  ...
  // 팀별 선수 목록 조회
  async getPlayersByTeam(teamId: string): Promise<PlayerNewDocument[]> {
    return this.playerNewModel.find({ 'team.id': teamId }).exec();
  }

  // 포지션별 선수 목록 조회
  async getPlayersByPosition(position: string): Promise<PlayerNewDocument[]> {
    return this.playerNewModel.find({ 'profile.position': position }).exec();
  }

  // 선수 번호로 조회
  async getPlayerByNumber(teamId: string, number: number): Promise<PlayerNewDocument | null> {
  ...
  // 계정 ID로 조회
  async getPlayerByAccountId(accountId: string): Promise<PlayerNewDocument | null> {
  ...
  async searchPlayers(searchTerm: string): Promise<PlayerNewDocument[]> {
  ...
  // 활성 선수만 조회
  async getActivePlayers(): Promise<PlayerNewDocument[]> {
